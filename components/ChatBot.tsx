...
const Chatbot = () => {
  ...
  const [chat, setChat] = useState<{
    balances: any; sender: string; text: string; type: string;validators:any ; contractInput:any;send:any;
}[]>([]);
  ...
  const confirmSend = async (sendDetails:any)=>{
    try{
      ...
      setChat((prevChat) => [
        ...prevChat,
        { sender: "ai", text: `Transfer success ! Here is your tx Hash : ${res.txHash}`, type: "text",intent:"general",balances:null,validators:null,contractInput:null ,send:null},
      ]);
      }else{
        const amount = {
      ...
      setChat((prevChat) => [
        ...prevChat,
        { sender: "ai", text: `Transfer failed, Error : ${error}`, type: "text",intent:"general",balances:null,validators:null,contractInput:null ,send:null},
      ]);
      console.log(error)
      return
    }
  ...
  const confirmSwap = async (contractInput:any) =>{
    try{
      ...
      setChat((prevChat) => [
        ...prevChat,
        { sender: "ai", text: `Swap success ! Here is your tx Hash : ${res.txHash}`, type: "text",intent:"general",balances:null,validators:null,contractInput:null ,send:null},
      ]);
      console.log(res)
      } else {
        const msg = MsgExecuteContractCompat.fromJSON({
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes('minimum receive amount')) {
          setChat((prevChat) => [
            ...prevChat,
            { sender: "ai", text: `Swap failed, Error : 'The swap failed because your minimum receive amount is too high. ' +    
            'Please adjust your slippage settings at your .env to proceed with the swap.'`, type: "text",intent:"general",balances:null,validators:null,contractInput:null ,send:null},
          ]);
          
        } else {
          setChat((prevChat) => [
            ...prevChat,
            { sender: "ai", text: `Swap failed, Error : ${errorMessage}`, type: "text",intent:"general",balances:null,validators:null,contractInput:null ,send:null},
          ]);
        }
      } else {
        ...
        setChat((prevChat) => [
          ...prevChat,
          { sender: "ai", text: `Swap failed, Error : ${error}`, type: "text",intent:"general",balances:null,validators:null,contractInput:null ,send:null},
        ]);
  ...
  const confirmStake = async () =>{
    ...
    setChat((prevChat) => [
      ...prevChat,
      { sender: "ai", text: `Stake success ! Here is your tx Hash : ${res.txHash}`, type: "text",intent:"general",balances:null,validators:null,contractInput:null,send:null },
    ]);
  ...
  const handleValidatorSelection = async (validatorIndex:any,name:string,validator:any) => {
    try {
      const res = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: `${validatorIndex}`, chatHistory: chat,address:injectiveAddress,intent: "stake_inj_amount"}),
      });
      ...
      setChat((prevChat) => [
        ...prevChat,
        { sender: "ai", text: `Validator #${name} selected`, type: "text",intent:"stake_inj_amount",balances:null,validators:null,contractInput:null,send:null },
      ]);
      ...
      setChat((prevChat) => [...prevChat, { sender: "ai", text: "Error processing request", type: "error",balances:null,validators:null,contractInput:null,send:null }]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExit = async () => {
    ...
    const exitToolMessage = { sender: "ai", text: "Tool closed successfully.", type: "text" ,balances:null,validators:null,contractInput:null,send:null};
  ...
  }, [chat]); // Runs whenever chat updates

  const sendMessage = async () => {
    if (!message.trim()) return;
  
    const newUserMessage = { sender: "user", text: message, type: "text" ,balances:null,validators:null,contractInput:null,send:null};
    ...
    <div className="flex h-screen w-screen bg-black text-white">
                  ...
                  <div className="relative">
                    ...
                    {showPopup && (
                      <div className="absolute top-12 left-0 bg-gray-800 text-white p-3 rounded-lg shadow-lg">
                        <button onClick={handleDisconnect} className="hover:text-red-500">
                          Disconnect
                        </button>
                      </div>
                    )}
      ...
      <main className="flex flex-col w-4/5">
        <div className="p-6 text-center">
          <h2 className="text-3xl font-semibold">JECTA <span className="text-xl text-gray-400">v0.0.2</span></h2>
        </div>
  
        {/* Chat Messages */}
              <div ref={chatContainerRef} className={`flex-1 bg-zinc-900 p-6 mx-6 mb-4 rounded-xl overflow-y-auto flex flex-col ${loading || executing ? 'animate-neonBlink' : ''}`}>
        {chat.filter((msg) => msg.sender !== "system").map((msg, i) => {
          
          // Detect if this is the last error message
          const isLastError = (msg.type === "error" || msg.type === "validators" || msg.type == "stake_amount" || msg.type == "swap"|| msg.type == "send_token") && i === chat.length - 2;
          ...
          return (
            <div key={i} className={`flex my-2 ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
              {msg.sender == "ai" && <img src={logo.src} alt="Logo" className="w-8 h-8 rounded-md mr-2 border-white border-1" />}
              
              {/* ✅ Handle Balance Messages */}
              {msg.type === "balance" ? (
                <div className="p-3 rounded-xl bg-zinc-800 text-white">
                  <div className="flex flex-col gap-3">
                    {msg.balances.map((token: { logo: string; symbol: string; balance: string; address: string }, index: Key | null | undefined) => (
                      <div
                        key={index}
                ...
                <>
                {isLastError ? (<><div className="p-3 rounded-xl bg-zinc-800 text-white max-w-[75%]">
                  <h3 className="text-lg font-semibold mb-2">Choose a Validator:</h3>
              
                  {/* ✅ Grid Layout for Validators */}
                  <div className="grid grid-cols-4 gap-3">
                    {msg.validators.map((validator: { moniker: string; address: string; commission: string }, index: number) => (
                      <button
                        key={index}
                        onClick={() => {
                ...
                <div className="p-3 rounded-xl bg-zinc-800 text-white max-w-[75%]">Selecting Validator...</div>
                </>)}
                </>
              ...
              {isLastError ? (<div className="p-3 rounded-xl bg-zinc-800 text-white max-w-[75%]">
                ...
                </div>):(<><div className="p-3 rounded-xl bg-zinc-800 text-white max-w-[75%]">
                ...
                {isLastError ? (<div className="p-3 rounded-xl bg-zinc-800 text-white max-w-[75%] " >
                    ...
                    <div>{msg.text}</div>
                    {!executing && (<div className=" space-x-4">
                        <button
                        onClick={handleExit}
                        className="mt-3 px-4 py-2 bg-white text-red-700 font-semibold rounded-lg hover:bg-gray-300"
                      >
                ...
                <div
                  className={`p-3 rounded-xl max-w-[75%] ${
                    msg.sender === "user" ? "bg-white text-black self-end" : "bg-zinc-800 text-white self-start"
                  }`}
                  style={{
        ...
        <div className="px-6 pb-6 flex items-center gap-3">
          <input
            ...
            onChange={(e) =>{
              ...
              console.log(loading)
              if (!(validatorSelected || lastMessageType === 'swap' ||lastMessageType === 'send_token'||  loading || executing || lastMessageType === "validators")) {
