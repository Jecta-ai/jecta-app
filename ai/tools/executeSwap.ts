...
export async function extractSwapDetails(message: string) {
    ...
    if (match) {
        ...
        return { from:from,from_metadata:fromMetaData, to:to,to_metadata:toMetaData, amount:amount, status:status }; // ✅ Correct object structure
    } else {
        const status = "failed";
...
export const fetchSwapDetails = async (fromMetaData: any,amount:number,toMetaData:any) => {
  ...
  const injectiveMainnetChainId = "injective-1";
      ...
      console.log("Route:", contractInput);
      ...
      const msg = `Route: ${route[0].dex} | Amount: ${amount} ${fromMetaData.symbol.toUpperCase()} = ${Number(minimumReceive)/10**toMetaData.decimals} ${toMetaData.symbol}`
