...
async function fetchPools(factoryName: string, contract: string) {
  ...
  if (!factory || !contract) {
      console.error(`⚠️ Invalid factory or contract address. Factory: ${factory?.address}, Contract: ${contract}`);
      return null; 
  }

  const baseAssets = [
      { native_token: { denom: "inj" } }, 
      { token: { contract_addr: contract } }
  ];
  ...
  export async function getPieChartData(data: Holder[], total_supply: number): Promise<any> {
    ...
    const othersPercentage = sortedData.slice(10).reduce((sum, holder) => sum + holder.percentage_held, 0);
            ...
            <tbody>
                ...
                .map((item) => {
                    ...
                    <tr>
                        ...
                        <td style="padding: 8px; border-bottom: 1px solid #047857; font-weight: bold; color: ${label ? "#34d399" : "#a7f3d0"};">
                        ${label ? label : "—"}
                        </td>
  ...
  export const fetchTokenMetadata = async (ticker: string) => {
    try {
      ...
      const tokenMetadata = response.data.find(
        (token: any) => token.symbol === ticker && (token.tokenType === "cw20" || token.tokenType === "tokenFactory")
    );
